...
public:
    ...
    bool isValid() const {
        ...
        for (auto i : std::views::iota(1uz, (size / 2) + 1)) {
            auto parts = str | std::views::chunk(i) | to<std::string_view>{} | std::views::pairwise;
            if (std::ranges::all_of(parts, [](const auto& p) { return std::apply(std::equal_to<>(), p); })) {
                return false;
            }
        }
